...
Perturber properties presented in previous section were constrained by the spatial distribution of the GD-1 debris alone.
...
Details of the loop proper motion offsets still depend on the perturber's parameters, but the directions of offsets along the stream are universal in the encounter scenario.
...
Finally, the magnitude of kinematic offsets across the stream ($\Delta\mu_{\phi_2}$) is universally smaller than along the stream ($\Delta\mu_{\phi_1}$).
...
- not true N-body (last stage of disruption )
- stream==orbit (just track, no density; don't expect to be very wrong as the same fiducial in streakline and orbit works)
- pseudo-likelihood (conservative: accepting wider range of models, see model B other side of the loop sticking out), some things may be wrong if likelihood is wrong
- not a comprehensive search of the parameter space, different islands in the parameter space may still be allowed (say lower mass); always true; low dimensional, could almost brute-force it (for future, with actual likelihood)
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.9\textwidth]{satellite_distances.pdf}
\end{center}
