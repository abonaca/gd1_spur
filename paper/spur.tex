...
- assumption: stream==orbit
- fiducial in this case becomes -> second row
- same values, but different orientation
- since stream-orbit misalignment is not negligible for GD-1 \citep{sb2013}, the orientation angles not the same
- a few more examples

- to understand where the constraints are coming from, we show the a grid of stream models that vary a single parameter at the time
- fiducial in the middle row
- from left to right: time, mass, impact parameter, velocity, size

- analytical argument for a young spur?
- slow v: no longer in the impulse approximation -- the exact degeneracy no longer holds there?
- this is not a comprehensive search of the parameter space, so lower mass solutions may still exist
...
Ideally, to find parameters allowed by the data, we would like to create a model of the stream and compare it to the data directly.
However, generating a realistic stream model is expensive, so instead, we devised a set of expert criteria that allow us to compare whether a conceptual stream model represents the observed structure of GD-1.
% - ABC
In the top row of Figure~\ref{fig:corner} we show the data used to evaluate models:
- describe data: gap == top hat; spur == spline through control points
- pseudo-likelihood: $\chi^2_{gap}$ + $\chi^2_{spur}$

- sample the space
- highly idealized, so we only provide plausible ranges
- marginalized over orientation angles, impact location

- takeaways from the corner plot: close encounter, recent encounter, solutions for a range of velocities, mass--velocity degeneracy, compact perturber
...
\subsection{Perturbative origin of GD-1 structures}
...
\subsection{Non-perturbative origin of GD-1 structures}
