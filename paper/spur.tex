...
The Milky Way is surrounded by $\sim60$ dwarf galaxies \citep{mcconnachie2012,etal} and $\sim160$ globular clusters \citep{harris2010,etal}, most of which, same as GD-1, are residing in the halo.
...
To test whether any of these objects could have perturbed GD-1, we integrated their orbits in a fiducial Milky Way potential \citep{pw2017} and show their relative distance from the GD-1 gap at $\phi_1=-40^\circ$ during the past Gyr in Figure~\ref{fig:known_encounters}.
Darker shades represent more massive satellites, from the most massive classical dwarfs, over ultra-faint dwarfs to globular clusters.
- nothing comes closer than 1\,kpc
Relative distances shown in Figure~\ref{fig:known_encounters} were calculated for fiducial positions and velocities of satellites.
...
Maximum impact parameter inferred is y\,pc, thus disfavoring known satellites as a source of GD-1 perturbation.

% but none of the satellites comes close enough even after sampling uncertainties
...
- the only thing that comes close is the Galactic disk
- disk shocking can facilitate disruption \citep{dehnen2004}, but the density contrast in the disk is not high enough to produce sharp features such as the GD-1 gap and spur
- \citet{amorisco2016} showed that gmcs in the disk can introduce 
- to see if viable to have produced GD-1 features, in Figure~\ref{fig:mass_size} we the expected mass and size of the GD-1 perturber, and compare it to masses and sizes of dwarf galaxies, globular clusters and giant molecular clouds
- while globular clusters and some compact dwarf galaxies have the right mass and size, gmcs are too fluffy
- just to check a bit further, we created GD-1 models perturbed by a $10^7\,\rm M_\odot$ gmc moving on a circular orbit for the three most recent disk crossing times
- none of the configurations result in a spur at the right location, thus disfavoring gmcs as the GD-1 perturber

Having ruled out known objects, we find that the most probable GD-1 perturber is a dark object in the Milky Way halo.
- could be a low luminosity, undiscovered satellite as they have approximately the required masses and sizes
- it would have to be fortuitously hiding in the disk plane, or moving very fast to escape detection, as our best estimate is that the encounter was recent
- alternatively, the perturber could be completely dark
- cosmologically sound expectation would be a dark matter subhalo
- but since we infer a dense perturber, it could also be a black hole
- very massive black hole, typically found in centers of galaxies (GD-1 perturber is comparable in mass to Sgr A$^\star$)
- given the abundance of massive primordial black holes from ? is low \citep{}, this encounter with GD-1 would be a rare event, and we would not expect to see similar features in other streams upon a comparable amount of scrutiny

- on the other hand, many subhalos are expected
- despite depletion \citep{gk2017}, GD-1 is expected to have encountered a few \citep{erkal}
- with two prominent gaps, and ascribing one to the progenitor disruption, one due to the subhalo works out in terms of rates
- however, inferred perturber a factor of a few more dense than the concentration for isolated halos (dark shaded region in Figure~\ref{fig:mass_size} with 0.15\,dex scatter)
- would explain how it survived disruption in the tidal field of the inner galaxy, but is at the very limit of high concentration tail in low mass halos \citep{diemer2015}
- even though they experience stripping, this should not really affect the rs \citep{arthur}
- details of the profile would matter here, so further work required in modeling, but most likely, we will need additional data to fully constrain it
- cosmological significance of a low-mass subhalo detection strongly motivates follow up, and we provide specific strategies in Section~\ref{sec:future}
...
The stream either encountered a likely exotic perturber, has been shaped by a curious dynamical effect, or originates from an unusual progenitor.
...
\label{sec:future}
- need rv, mu
- assemble all the predictions (bulleted list)
- get better orbit of the plausible object 

- implications: black hole - bound stellar cluster, some evidence of accretion; dark matter - possible stellar cluster too, annihilation (dense); mass profile to distinguish the two if not accreting or annihilating (microlensing)
- if black hole:
- network of streams: image dark matter in 3+1D
