...
\label{sec:future}
- we think perturbation by a dm subhalo is the most likely, however inferred properties not entirely expected
- 


% - a puzzle, with no explanation that readily explains all facets of the data, so more theoretical work needed, as outlined in Section~\ref{sec:caveats}
% Here we turn our mind to additional observational tests and imagine what might be possible.
% 
% We propose to:
% \begin{enumerate}
% \item{
% \emph{Measure radial velocities:}
% - this will constrain the orbit of the perturber, narrow down search area
% - a large relative energy between the spur and the stream would argue for an external influence, rather than a feature in the progenitor
% }
% \item{
% \emph{Measure precise proper motions:}
% - encounter has a unique signature, scenario can be falsified
% }
% \item{
% \emph{Search for perturber via other means:}
% - perturber massive, should have signatures, for example can have a few stars bound, microlensing signal
% - if black hole, perhaps some evidence of accretion, for some types of dark matter we might have annihilation
% - if perturber found, ultimately can distinguish between these from the detailed density profile (with microlensing), independent of the accretion state and the nature of dark matter
% }
% \end{enumerate}

% GD-1 stream is a beautiful display of information encoded in cold stellar streams, whether it be on their progenitor or their environment.
% - enabled by gaia
% - and this is just the beginning, there are more than 40 streams (and also the blob feature in GD-1!), orbiting between 5 and 40\,kpc
% - unparalleled opportunity to map matter in the 3 spatial dimensions and time
% - signal that dark matter mass function extension to masses below $10^8\,\rm M_\odot$ might be around the corner
