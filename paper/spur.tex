...
% - baseline model that reproduces the models
...
\label{sec:data}
GD-1 is the longest ($>100^\circ$, kpc) thin ($\sigma\approx12'$, pc) stellar stream discovered in the Galactic halo \citep{gd2006}.
...
We then modeled the spatial distribution of these stars in the $\phi_2$ direction (i.e. along the stream) as a combination of a constant background, a stream component at the location of the main stream track, and two additional Gaussian components on either side of the main stream.
- free parameters, constrained w mcmc
- for each star, we calculate
- total membership is
...
- stream width and density change along the stream
- most prominent features: two gaps, one looks almost completely empty
The number of stars in the additional components is larger than the background in the spur and blob region, and consistent with zero along the rest of the stream.
In the following section we present a model of GD-1 that simultaneously explains both the gap in the stream and the associated spur of member stars.

\subsection{Perturbed model of GD-1}
\label{sec:model}
% - gaps studied as a signature of interaction
% - in addition, here we have stars off from the main stream
% - in this section we show that both features can be simultaneously explained with an interaction
- unlike the observed gd-1, model in a smooth galaxy is also smooth \citep{pwb}
- streakline model \citep{fardal2015}, so it does account for substructure inherent to the process of disruption \citep{kupper2008,kupper2010}
...
\acknowledgements
It is a pleasure to thank Benedikt Diemer, Elena D'Onghia, Douglas Finkbeiner, Lars Hernquist, Kathryn Johnston, Doug Lin, Erica Nelson, Sarah Pearson, Hans-Walter Rix, and Josh Speagle + someone from CUNY for valuable discussions and input.
